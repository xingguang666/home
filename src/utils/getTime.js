...
// 欢迎提示
export const helloInit = async () => {
  try {
    // 调用API获取问候语
    const { getGreetingMessage } = await import("@/api/index.js");
    const response = await getGreetingMessage();

    if (response && response.code === 200 && response.data) {
      // 使用API返回的问候语
      ElMessage({
        dangerouslyUseHTMLString: true,
        message: `<strong>${response.data.greeting}</strong> ${response.data.tip || "欢迎来到我的主页"}`,
      });
    } else {
      // API调用失败，使用本地逻辑作为备用
      fallbackGreeting();
